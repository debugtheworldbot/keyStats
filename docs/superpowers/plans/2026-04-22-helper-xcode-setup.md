...
- **目录：** `helper/` 挂 synchronized root group（Xcode 15+），自动同步目录下所有文件到两个 target

### 目录里文件归属

`helper/` 目录下的文件通过 `PBXFileSystemSynchronizedBuildFileExceptionSet` 控制：
...
| `CODE_SIGN_ENTITLEMENTS` | `helper/KeyStatsHelper.entitlements` |
...
- **Synchronized Root Group 会自动把目录下所有文件加到 target。** 不要把 Shared 文件放到 `KeyStats/Shared/` 再手动拖进 Xcode——Xcode 15+ 会复制一份到目标 group 对应的目录。直接把 Shared 文件放到 `helper/` 目录里，利用 exception set 让它们在 KeyStats target 里也可见，省事。
