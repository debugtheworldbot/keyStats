...
export function validateDeviceToken(value: unknown, expectedDeviceId: string, field = "deviceToken"): string {
  if (typeof value !== "string" || value.length > 160 || value.includes("\n")) {
    throw new Error(`${field} is not a valid device token`);
  }
  ...
  if (tokenDeviceId.toLowerCase() !== expectedDeviceId.toLowerCase() || !SHA256_BASE64URL_RE.test(secret)) {
    throw new Error(`${field} must be bound to deviceId`);
  }
  return value;
}
...
export function decodeBase64(value: unknown, field: string, maxBytes: number): Uint8Array {
  if (typeof value !== "string" || value.length === 0 || value.length > Math.ceil(maxBytes / 3) * 4 + 4) {
    throw new Error(`${field} is not a valid base64 value`);
  }
  if (!BASE64_RE.test(value)) throw new Error(`${field} is not canonical base64`);
  let binary: string;
  try {
    binary = atob(value);
  } catch {
  ...
  if (binary.length > maxBytes) throw new Error(`${field} exceeds ${maxBytes} bytes`);
  const output = new Uint8Array(binary.length);
  for (let index = 0; index < binary.length; index += 1) output[index] = binary.charCodeAt(index);
  if (bytesToBase64(output) !== value) throw new Error(`${field} is not canonical base64`);
  return output;
}
...
export function validateEnvelope(value: unknown, field = "envelope", maxCiphertextBytes = 16_384): EncryptedEnvelope {
  ...
  if (nonce.length !== 12) throw new Error(`${field}.nonce must decode to 12 bytes`);
  const ciphertext = decodeBase64(candidate.ciphertext, `${field}.ciphertext`, maxCiphertextBytes);
  if (ciphertext.length === 0) throw new Error(`${field}.ciphertext cannot be empty`);
  const tag = decodeBase64(candidate.tag, `${field}.tag`, 16);
  if (tag.length !== 16) throw new Error(`${field}.tag must decode to 16 bytes`);
  return {
...
export async function validateRecord(value: unknown, expectedDeviceId: string): Promise<EncryptedRecord> {
  ...
  if (candidate.schemaVersion !== 1) throw new Error("record.schemaVersion must equal 1");
  if (typeof candidate.recordId !== "string" || !/^[A-Za-z0-9_-]{43}$/.test(candidate.recordId)) {
    throw new Error("record.recordId must be an opaque base64url identifier");
  }
  if (candidate.deviceId !== expectedDeviceId) throw new Error("record.deviceId must match the authenticated device");
  ...
  if (typeof candidate.ciphertextHash !== "string" || !SHA256_BASE64URL_RE.test(candidate.ciphertextHash)) {
    throw new Error("record.ciphertextHash must be a base64url SHA-256 digest without padding");
  }
  ...
  if (!timingSafeEqual(computedHash, candidate.ciphertextHash)) {
    throw new Error("record.ciphertextHash does not match nonce || ciphertext || tag");
  }
  return {
    schemaVersion: 1,
...
export function validateRawPublicKey(value: unknown, field: string): string {
  ...
  if (key.length !== 32) throw new Error(`${field} must decode to a raw 32-byte X25519 public key`);
  return value as string;
}

export function validateRecoveryAuthToken(value: unknown): string {
  if (typeof value !== "string" || !/^[A-Za-z0-9_-]{43}$/.test(value)) {
    throw new Error("recoveryAuthToken must be a base64url-encoded 32-byte value without padding");
  }
  return value;
}
...
function assertExactKeys(candidate: Record<string, unknown>, field: string, allowedKeys: readonly string[]): void {
  ...
  if (unexpected) throw new Error(`${field}.${unexpected} is not allowed`);
}
